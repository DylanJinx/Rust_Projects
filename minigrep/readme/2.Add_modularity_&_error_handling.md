...
#### 1.3.3.1 unwrap_or_else
在 Rust 中，`unwrap_or_else` 是 `Option` 和 `Result` 类型的一个方法，用于在处理可能的错误或空值时提供一个替代方案。这个方法允许你指定一个闭包（一个匿名函数），当 `Option` 是 `None` 或者 `Result` 是 `Err` 时执行，从而提供一个默认值或进行错误处理。

##### 1.3.3.1.1 `Option` 类型的 `unwrap_or_else`
对于 `Option` 类型，`unwrap_or_else` 方法允许在 `Option` 是 `None` 时执行一个闭包来提供一个默认值。如果 `Option` 是 `Some`，则返回包含的值。

```rust
let option = Some("Hello");
let result = option.unwrap_or_else(|| "Default");
println!("{}", result);  // 输出 "Hello"

let option: Option<&str> = None;
let result = option.unwrap_or_else(|| "Default");
println!("{}", result);  // 输出 "Default"
```

在这个例子中，第一个 `option` 是 `Some` 类型，所以 `unwrap_or_else` 直接返回 `"Hello"`。第二个 `option` 是 `None`，所以 `unwrap_or_else` 执行闭包，返回 `"Default"`。

##### 1.3.3.1.2 `Result` 类型的 `unwrap_or_else`
对于 `Result` 类型，`unwrap_or_else` 方法在 `Result` 是 `Err` 时执行闭包，该闭包接收 `Err` 中的错误值，并返回一个新的值。如果 `Result` 是 `Ok`，则直接返回成功值。
...
在这个例子中，当 `result` 是 `Ok(10)`，`unwrap_or_else` 直接返回 `10`。当 `result` 是 `Err("Something went wrong")` 时，闭包被执行，打印错误信息，并返回 `-1` 作为默认值。
...
- `dyn` 关键字用来表示动态分派。这是一种多态性的形式，允许在运行时决定调用哪个具体的方法。当你使用 `dyn` 关键字时，你通常是在处理特征对象（trait objects）。特征对象让你可以在运行时处理不同类型的值，只要它们实现了相应的特征。
- `Box<dyn trait>`
	- 在 Rust 中，函数的返回值通常存储在栈上，而栈上的数据需要在编译时具有已知的固定大小。这是因为栈的内存分配非常快速，但需要事先知道每个项目的大小，以便为其分配空间。
	- 对于 `dyn Trait`（动态特征对象），其大小在编译时是未知的，因为 `dyn Trait` 表示任何实现了该特征的类型，而不同的类型可能有不同的大小。因此，直接在栈上存储 `dyn Trait` 是不可行的。
	- 使用 `Box<dyn Trait>` 是一种常见的解决方案，因为 `Box` 是一个智能指针，它在堆上分配内存。尽管 `dyn Trait` 的大小未知，但 `Box<dyn Trait>` 的大小是已知的（即指针的大小），因此它可以安全地存储在栈上。通过这种方式，你可以在运行时动态处理不同类型的对象，同时保持编译时类型和大小的确定性。
